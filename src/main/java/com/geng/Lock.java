...
/**
 * @author GengYongjiang on 2018/12/6
 * 乐观锁与sychronized测试
 */
...
public class Lock extends Thread {
    ...
    static {
            try {
                ...
                casOffset = UNSAFE.staticFieldOffset(Lock.class.getDeclaredField("cas"));
            } catch (Exception e) {
                e.printStackTrace();
            }
    }
    ...
    public void cas(){
         ...
         while(!bl){
             tmp = cas;
             bl = UNSAFE.compareAndSwapInt(Lock.class, casOffset, tmp,tmp+1);
         }
    }
